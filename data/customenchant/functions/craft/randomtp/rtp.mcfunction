tag @s add Craftrtp
execute at @e[tag=Craftrtp] run summon item ~ ~ ~ {Item:{id:"minecraft:diamond_leggings",Count:1b,tag:{Customtag:"rtp",Damage:0,display:{Lore:['{"text":"Random TP","color":"light_purple"}']}}}}
...
execute at @e[tag=Craftrtp] run tag @s remove CraftTB