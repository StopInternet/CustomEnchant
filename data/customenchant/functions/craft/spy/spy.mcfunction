tag @s add CraftSpy
execute at @e[tag=CraftSpy] run summon item ~ ~ ~ {Item:{id:"minecraft:iron_leggings",Count:1b,tag:{Customtag:"spy",Damage:0,display:{Lore:['{"text":"Spy","color":"light_purple"}']}}}}
...
execute at @e[tag=CraftSpy] run tag @s remove CraftTB